...
import babel.runtime.exceptions.HandlerRegistrationException;
import babel.genericprotocol.handlers.*;
import babel.runtime.protocol.*;
import babel.runtime.events.consumers.ProtoConsumers;
...
public abstract class GenericProtocol implements ProtoConsumers {
            ...
            try {
                ...
                switch (pe.getType()) {
                    ...
                    case CUSTOM_CHANNEL_EVENT:
                        ...
                        break;
                    default:
                        throw new AssertionError("Unexpected event received by babel.runtime.protocol "
                                + protoId + " (" + protoName + ")");
                }
    ...
    private void handleMessageFailed(MessageFailedEvent e) {
        ProtoMessage msg = e.getMsg().getMsg();
        MessageFailedHandler h = getChannelOrThrow(e.getChannelId()).messageFailedHandlers.get(msg.getId());
        if (h != null)
            h.onMessageFailed(msg, e.getTo(), e.getMsg().getDestProto(), e.getCause(), e.getChannelId());
    }
    ...
    private void handleMessageSent(MessageSentEvent e) {
        ...
        if (h != null)
            h.onMessageSent(msg, e.getTo(), e.getMsg().getDestProto(), e.getChannelId());
    }
